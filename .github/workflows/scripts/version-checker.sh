...
# We don't have node in this plugin
...
README_VERSION="$(grep -E '^Stable tag:' ./plugin/readme.txt | head -n1 | sed -E 's/^Stable tag:[[:space:]]*//')"
...
PLUGIN_VERSION="$(grep -E '^[[:space:]]*\*?[[:space:]]*Version:[[:space:]]*[0-9]+\.[0-9]+\.[0-9]+' ./plugin/ph-content-relations.php \
  | head -n1 \
